...
import { EditPlayerProps } from "../store/interfaces/user.interface";
import {
...
import { EditPlayerSchema } from "../utils/Yup";
import {
  FootballPosition,
  BasketballPosition,
  VolleyballPosition,
...
const EditPlayerModal = ({
  ...
  const [isEditPlayerLoading, setIsEditPlayerLoading] = useState(false);
  ...
  const selectedTeamm = teamsData?.find(
    (team: any) => team._id === playerData?.team?._id
  );

  let formattedValue = "";
  useEffect(() => {
    if (selectedTeamm) {
      setSelectedTeamSport(selectedTeamm?.sport);
      formattedValue = selectedTeamm
        ? `${selectedTeamm.name} - ${selectedTeamm.sport} - ${selectedTeamm.division}`
        : "";
    }
    console.log(formattedValue);
  }, [selectedTeamm]);

  // Format the selected team information

  const [
    editPlayer,
    { error: editPlayerError, isError: editPlayerIsError },
  ]: any = useUpdatePlayerMutation();

  console.log(editPlayerError);
  ...
  const handleEditPlayer = useCallback(
    async (props: EditPlayerProps) => {
      setIsEditPlayerLoading(true);
      ...
      try {
        const response = await editPlayer(inputValue);
        ...
        setIsEditPlayerLoading(false);
      }
    },
    [
      editPlayer,
      setIsEditPlayerLoading,
      setOpenModal,
      setOpenStatusModal,
      setOpenStatussModal,
    ]
    ...
    <>
      ...
      >
          ...
          <Modal.Body className="overflow-y-visible flex flex-col justify-center w-full max-w-[62rem] bg-[#ffffff] gap-8 rounded-[1.1875rem] px-8 pb-8">
            <Formik
              initialValues={{
                ...
                position: playerData?.position, // Default value for dropdown
                playerRole: playerData?.playerRole, // Default value for dropdown
                profileImage: playerData?.profileImage, // Default value for dropdown
              }}
              ...
              {({ errors, values, setFieldValue }) => (
                <Form className="flex w-full flex-col gap-7 overflow-auto">
                  {editPlayerIsError && (
                    <Alert color="failure" className="py-3">
                      <span className="font-medium" id="error-alert">
                        {editPlayerError && editPlayerError?.data?.message}
                      </span>
                    </Alert>
                  )}
                  ...
                  <div className="w-full flex flex-col gap-3 md:gap-0 md:flex-row">
                    ...
                    <div className="flex flex-col w-full">
                      <select
                        ...
                        value={
                          seasonsData?.find(
                            (season: any) => season._id === values.season
                          )?.season
                        }
                        disabled
                      >
                  ...
                  <div className="flex border-0 justify-between w-full">
                    ...
                    <button
                      className={`${
                        isEditPlayerLoading ? "bg-white" : "bg-custom-primary-1"
                      }  ${
                        isEditPlayerLoading
                      ...
                      disabled={isEditPlayerLoading}
                      type="submit"
                    >
                      {isEditPlayerLoading ? <ButtonSpinner /> : "Edit"}
                    </button>
                  </div>
                </Form>
              )}
        ...
        <AddItemSuccessModal
          ...
          message={"You have successfully edited a player."}
        />
        <AddItemErrorModal
