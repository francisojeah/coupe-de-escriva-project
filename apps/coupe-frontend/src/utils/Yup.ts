...
import { BasketballPosition, FootballPosition, GameWeek, VolleyballPosition } from './constants';
...
export const AddMatchStatsSchema = Yup.object().shape({
  ...
  selectedNumber: Yup.number().required('Number is required').positive('Number must be positive').integer('Number must be an integer')
});
...
export const AddPlayerSchema = Yup.object().shape({
  ...
  playerRole: Yup.string().oneOf(['Captain', 'Player', 'Assistant Captain' ]).required('Player role is required'),
  team: Yup.string().required('Team is required'),
...
export const AddFixtureResultSchema = Yup.object().shape({
  ...
  fixtures: Yup.object().shape({
    ...
    gameweek: Yup.mixed().oneOf(Object.values(GameWeek)).required('Gameweek is required'),
  }),
});
